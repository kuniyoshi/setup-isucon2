...
use Cache;
my $cache;
$cache = Cache->new if $ENV{CACHE_MODE};
...
push @bufs, join( $CRLF, @headers, "" ) . $CRLF;
    syswrite $conn, $bufs[-1];
...
#        syswrite $conn, "0$CRLF$CRLF" if $chunked;
        if ($chunked) {
            syswrite $conn, "0$CRLF$CRLF";
push @bufs, "0$CRLF$CRLF";
        }
use Data::Dumper;
(my $last = join q{}, @bufs) =~ s{^HTTP/1.1 200 OK\n}{};
#warn Dumper $last;
use utf8;
$cache->set($env->{PATH_INFO}, $last) if $ENV{CACHE_MODE};
    } else {
